...
class Timer {
   ...
   public:
    ...
    long long stop() {
        ...
        return std::chrono::duration_cast<std::chrono::seconds>(timeEnd - mTimeStart).count();
    }
...
std::pair<double, double> p2p_host_to_ssd(int& nvmeFd, xrtDeviceHandle device, xrt::kernel& krnl, xrt::bo bo0, xrt::bo p2p_bo1, int* p2p_bo1_map) {
	Timer timer_from_cpu, timer_from_fpga;
    int ret = 0;


    std::cout << "Start timer" << " : " << global_timer.stop() << std::endl;
...
std::pair<double, double> p2p_ssd_to_host(int& nvmeFd, xrtDeviceHandle device, xrt::kernel& krnl, xrt::bo bo_out, xrt::bo p2p_bo0, int* p2p_bo0_map) {
	Timer timer_from_cpu, timer_from_fpga;
    ...
    // Execution of the kernel
    auto run1 = krnl(p2p_bo0, bo_out);
    run1.wait();

    std::cout << "Get the output data from the device" << " : " << global_timer.stop() << std::endl;
...
int main(int argc, char** argv) {
    ...
    auto bo0_write = xrt::bo(device, vector_size_bytes, krnl.group_id(0));
    auto p2p_write_bo1 = xrt::bo(device, vector_size_bytes, flags, krnl.group_id(1));

    std::cout << "Map the contents of the buffer object into host memory : " << global_timer.stop() << std::endl;
    auto bo0_write_map = bo0_write.map<int*>();
    auto p2p_write_bo1_map = p2p_write_bo1.map<int*>();
    
    std::cout << "Fill the map" << " : " << global_timer.stop() << std::endl;
    std::fill(bo0_write_map, bo0_write_map + size, 1);
    std::fill(p2p_write_bo1_map, p2p_write_bo1_map + size, 1);

    std::cout << "Allocate Buffer in Global Memory : " << global_timer.stop() << std::endl;
    auto p2p_read_bo0 = xrt::bo(device, vector_size_bytes, flags, krnl.group_id(0));
    auto bo_read_out = xrt::bo(device, vector_size_bytes, krnl.group_id(1));

    std::cout << "Map the contents of the buffer object into host memory : " << global_timer.stop() << std::endl;
    auto p2p_read_bo0_map = p2p_read_bo0.map<int*>();
    auto bo_read_out_map = bo_read_out.map<int*>();

    std::cout << "Fill the map : " << global_timer.stop() << std::endl;
    std::fill(p2p_read_bo0_map, p2p_read_bo0_map + size, 0);
    std::fill(bo_read_out_map, bo_read_out_map + size, 0);
    ...
    for (int i=0; i<num_iter; i++) {
        ...
        auto p1 = p2p_host_to_ssd(nvmeFd, device, krnl, bo0_write, p2p_write_bo1, p2p_write_bo1_map);
        ...
        auto p2 = p2p_ssd_to_host(nvmeFd, device, krnl, bo_read_out, p2p_read_bo0, p2p_read_bo0_map);
    ...
    long long duration = timer.stop();// convert us to s;   
